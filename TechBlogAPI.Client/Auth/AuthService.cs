...
public class AuthService(HttpClient httpClient, IJSRuntime jsRuntime)
{
    ...
    public async Task<string> GetAccessTokenAsync()
    {
        ...
        if (string.IsNullOrEmpty(token) || string.IsNullOrEmpty(expirationStr)) return null;
        ...
        if (DateTime.TryParseExact(expirationStr, "o", CultureInfo.InvariantCulture,
            ...
            if (expiration <= DateTime.UtcNow.AddMinutes(1))
            {
                ...
                if (refreshed) return await jsRuntime.InvokeAsync<string>("localStorage.getItem", "accessToken");
